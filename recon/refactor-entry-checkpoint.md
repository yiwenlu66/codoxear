...
The pure-helper extraction wave and endpoint-controller extraction wave are closed. Launch-attempt record composition, direct/tmux process orchestration, tmux metadata polling, and launch process context construction are now separated from `spawn_web_session`. The active work is semantic ownership cleanup of the remaining non-route `server.py` responsibilities: next target launch dependency wiring or a larger launch precondition plan while preserving `SessionManager` authority over live resume checks, worktree creation, process monkeypatch seams, and registry consequences unless those authorities are deliberately moved with focused tests, Docker acceptance, and a clean-room review.
