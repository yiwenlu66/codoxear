...
The pure-helper extraction wave and endpoint-controller extraction wave are closed. Launch-attempt record composition, direct/tmux process orchestration, and tmux metadata polling are now separated from `spawn_web_session`. The active work is semantic ownership cleanup of the remaining non-route `server.py` responsibilities: next move launch context/dependency construction into an explicit plan object while preserving `SessionManager` authority over live resume checks, worktree creation, and registry consequences unless those authorities are deliberately moved with focused tests, Docker acceptance, and a clean-room review.
