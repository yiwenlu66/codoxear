...
- Launch-attempt record composition for web-owned spawn state transitions now lives in `codoxear.launch_ledger.LaunchAttemptRecorder`. The recorder owns base-copy/state/updated_ts/extra-merge/failure/stderr fallback mechanics; `SessionManager.spawn_web_session` still owns base launch context construction and server-specific `SessionLaunchError` raising.
- Direct/tmux web-session process orchestration now lives in `codoxear.session_launcher`. It owns direct `Popen` invocation, early-exit mapping, stderr drain/wait threads, tmux env/window/shell command construction, tmux retry attempts, pane-created/pending/broker-meta-bound launch records, and process-run return shapes. `SessionManager.spawn_web_session` remains the source-of-truth owner for cwd creation, worktree creation, resume validation, backend argv/env assembly, pane snapshot helper, and registry consequences.
- Tmux launch metadata polling now also lives in `codoxear.session_launcher.wait_for_spawned_broker_meta`. The server keeps `_wait_for_spawned_broker_meta` as a compatibility wrapper that injects `SOCK_DIR`; sidecar metadata reading and live-pid validation are explicit dependencies/defaults in the launcher helper.
- Launch process context construction now lives in `codoxear.session_launcher.prepare_launch_process_context`. It owns launch id/nonce generation, launch env marker mutation, base launch-attempt record fields, `LaunchAttemptRecorder` construction, and `LaunchProcessRequest` assembly. `SessionManager.spawn_web_session` passes a `LaunchContextRequest` after it has completed cwd/resume/worktree/backend argv/env preparation.
- Direct-process wait/drain mechanics now live in `codoxear.session_launcher.wait_or_raise` and `codoxear.session_launcher.drain_stream`. `server._wait_or_raise` and `server._drain_stream` remain compatibility wrappers and are still injected into `LaunchProcessDeps`, preserving existing patch seams.

Acceptance evidence for the latest discovery and launch-lifecycle tranches:
...
- `c5b7080 Extract session launch process runner`: focused local launch/source group -> `56 passed, 12 subtests`; local full -> `1077 passed, 107 subtests`; focused Docker on port 18951 passed; full Docker on port 18952 -> `1076 passed, 1 skipped, 107 subtests`; clean-room review `/tmp/codoxear-session-launcher-review.md` -> `NO BLOCKERS`.
- `0568b5b Cover session launcher edge cases`: tests-only follow-up for tmux success/failure/retry, direct spawn/stderr branches, bad metadata, and dead-pane metadata failure; focused local launch/source group -> `63 passed, 12 subtests`; local full -> `1084 passed, 107 subtests`; focused Docker on port 18953 passed; full Docker on port 18954 -> `1083 passed, 1 skipped, 107 subtests`.
- `0080b29 Move launch metadata wait helper`: focused local launch/source group -> `65 passed, 12 subtests`; import isolation check -> `server_imported False`; local full -> `1086 passed, 107 subtests`; focused Docker on port 18955 passed; full Docker on port 18956 -> `1085 passed, 1 skipped, 107 subtests`; clean-room review `/tmp/codoxear-metadata-wait-review.md` -> `NO BLOCKERS`.
- `ae26091 Extract launch process context`: direct launcher tests -> `15 passed`; focused local launch/provenance group -> `72 passed, 12 subtests`; local full -> `1088 passed, 107 subtests`; focused Docker on port 18959 -> `72 passed, 12 subtests`; full Docker on port 18960 -> `1087 passed, 1 skipped, 107 subtests`; clean-room review `/tmp/codoxear-launch-context-review.md` -> `NO BLOCKERS`.
- `9f51ab4 Move launch wait and drain helpers`: direct launcher tests -> `19 passed`; focused local launch/provenance group -> `76 passed, 12 subtests`; local full -> `1092 passed, 107 subtests`; focused Docker on port 18961 -> `76 passed, 12 subtests`; full Docker on port 18962 -> `1091 passed, 1 skipped, 107 subtests`; clean-room review `/tmp/codoxear-wait-drain-review.md` -> `NO BLOCKERS`.
...
The pure-helper extraction wave and endpoint-controller extraction wave are closed. Launch-attempt record composition, direct/tmux process orchestration, tmux metadata polling, launch process context construction, and direct-process wait/drain mechanics are now separated from `spawn_web_session`. The active work is semantic ownership cleanup of the remaining non-route `server.py` responsibilities: next target tmux runtime inspection or a larger launch precondition plan while preserving `SessionManager` authority over live resume checks, worktree creation, process/tmux monkeypatch seams, and registry consequences unless those authorities are deliberately moved with focused tests, Docker acceptance, and a clean-room review.
