...
`codoxear/static/app.js` is a 34-line bootstrap: it authenticates, creates the application controller, and selects login or application rendering. Composition lives in `app_application.js` (dependency facade), `app_application_composition.js` (lifecycle assembly), and `app_wiring.js` (per-controller `select()` contracts — no option bags, no spreads). Mutable session-runtime state lives in `app_session_state.js` (7-field observable store: `selected`, `running`, `queueLen`, `subagentsRunning`, `turnOpen`, `sending`, `token`). Session-list and defaults state lives in `app_session_catalog.js` (derived `sessionIndex`, atomic `applySnapshot`/`patchSession`). `app_polling.js` owns poll scheduling and async-epoch cancellation. Widgets follow a one-module rule: element + render + subscription in one file. `app_topbar.js` owns the status chip, context chip, and interrupt button end-to-end. `app_notifications.js` owns notification runtime. `app_help.js` owns the help overlay. `app_shell.js` owns only the `topMeta`, `titleRow`, and `topActions` layout slots; `app.css` owns presentation. Domain modules (`app_chat_interaction.js`, `app_file_ops.js`, `app_session_lifecycle.js`, `app_session_refresh.js`) and peripheral controllers (`app_message_flow.js`, `app_composer.js`, `app_unattended.js`, plus transcript, queue, file-viewer, launch, search, and voice modules) own their stateful workflows. An architecture guard (`scripts/check_wiring.py`, 7 check types, monotonic allowlist ratchet) prevents wiring regression. On the backend, log-derived session projection commits through one ordered boundary (`LogDerivedSessionObservation` + `commit_log_observation`); poll and SSE share one live-delta projection; `SessionManager` uses a retained coordinator graph with focused dependency records.
