...
## 2026-07-16 — User feedback 1: preview login logs out main service
- User observation: logging into the Docker preview causes the main Codoxear service to become logged out.
- Mechanism confirmed in source: both services use cookie name `codoxear_auth` with `Path=/` on the same hostname. Browser cookie identity excludes TCP port, so preview and main do not have separate cookie namespaces. Their HMAC secrets are correctly isolated, which makes the newly overwritten token invalid at the other service.
- Correction to prior isolation claim: process, filesystem, broker, socket, log, and network-listener isolation were proven; browser authentication state was not isolated. The shared cookie namespace is a real cross-service boundary defect.
- Prediction: giving the preview a distinct cookie name while keeping the production default unchanged will allow both login cookies to coexist on the same hostname and path. Logging into either service will no longer alter the other's authenticated state.
- Intervention dispatched: add validated `CODEX_WEB_COOKIE_NAME` configuration with default `codoxear_auth`; set preview to `codoxear_preview_auth`; test default/custom/invalid values; redeploy and verify both services in one browser profile.

## 2026-07-16 — Feedback 1 resolved: main and preview authentication coexist
- Functional commit `449c9cd` adds validated `CODEX_WEB_COOKIE_NAME`; absent or exactly empty retains production default `codoxear_auth`. Configured cookie names reject RFC control/separator characters, including leading/trailing whitespace, rather than silently normalizing. Preview launch sets `codoxear_preview_auth`. Documentation commit `7b9bda8` records the option and same-host multi-service use.
- Validation: focused server-config/auth suites passed (`20 passed`); full local suite passed (`1842 passed, 134 subtests`). Invalid space/control/separator probes all failed loud with exact offsets; default and preview names resolved correctly.
- Preview container was recreated only after `broker-514` was confirmed idle. Docker inspect shows `CODEX_WEB_COOKIE_NAME=codoxear_preview_auth`; health and tailnet HTTPS unauthenticated 401 remained correct.
- Decisive same-browser-profile proof: logged into main at HTTPS port 8443 and observed `/api/me` 200; opened preview at HTTPS port 19581 in a second tab and logged in; returned to main and `/api/me` remained 200 with app shell present; returned to preview and `/api/me` remained 200 with app shell present. This directly falsifies the observed cross-login logout mechanism.
- Existing user browsers still hold the old preview-signed `codoxear_auth` from before the fix. One fresh login to main replaces it with the correct main token; future preview logins use `codoxear_preview_auth` and no longer overwrite main.
