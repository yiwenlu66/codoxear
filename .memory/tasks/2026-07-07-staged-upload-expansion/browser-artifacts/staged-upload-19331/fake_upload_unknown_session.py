...
SOCKS = APP/'socks'
LOGDIR = HOME/'upload-unknown-logs'
CWD = HOME/'upload-unknown-repo'
CALLS = HOME/'upload-unknown-calls.jsonl'
for p in (SOCKS, LOGDIR, CWD): p.mkdir(parents=True, exist_ok=True)
(CWD/'README.md').write_text('upload proof repo\n', encoding='utf-8')
SID='upload-unknown'
log_path=LOGDIR/f'{SID}.jsonl'
rows=[
 {'type':'event_msg','ts':1.0,'payload':{'type':'user_message','message':'upload proof fixture'}},
...
sidecar={
 ...
 # Stage-only upload should work without key_write_errors; confirmed send still works.
  ...
  while True:
    ...
    try:
      ...
      if cmd=='state': resp=dict(state)
        ...
        if 'FORCE_COMMIT_UNKNOWN' in text:
          resp={'commit_unknown':True,'error':'forced unknown'}
        else:
