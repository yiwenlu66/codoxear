...
SOCKS = APP/'socks'
LOGDIR = HOME/'redaction-proof-logs'
CWD = HOME/'redaction-proof-repo'
CALLS = HOME/'redaction-proof-calls.jsonl'
for p in (SOCKS, LOGDIR, CWD): p.mkdir(parents=True, exist_ok=True)
(CWD/'README.md').write_text('redaction proof repo\n', encoding='utf-8')
SID='redaction-proof'
log_path=LOGDIR/f'{SID}.jsonl'
rows=[
 {'type':'event_msg','ts':1.0,'payload':{'type':'user_message','message':'redaction proof fixture'}},
