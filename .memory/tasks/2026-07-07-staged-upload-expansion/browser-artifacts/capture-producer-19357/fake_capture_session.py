...
SOCKS = APP/'socks'
LOGDIR = HOME/'capture-proof-logs'
CWD = HOME/'capture-proof-repo'
CALLS = HOME/'capture-proof-calls.jsonl'
for p in (SOCKS, LOGDIR, CWD): p.mkdir(parents=True, exist_ok=True)
(CWD/'README.md').write_text('capture proof repo\n', encoding='utf-8')
SID='capture-proof'
log_path=LOGDIR/f'{SID}.jsonl'
rows=[
 {'type':'event_msg','ts':1.0,'payload':{'type':'user_message','message':'capture proof fixture'}},
