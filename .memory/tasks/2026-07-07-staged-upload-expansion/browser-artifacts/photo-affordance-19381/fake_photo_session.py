...
SOCKS = APP/'socks'
LOGDIR = HOME/'photo-proof-logs'
CWD = HOME/'photo-proof-repo'
CALLS = HOME/'photo-proof-calls.jsonl'
for p in (SOCKS, LOGDIR, CWD): p.mkdir(parents=True, exist_ok=True)
(CWD/'README.md').write_text('photo proof repo\n', encoding='utf-8')
SID='photo-proof'
log_path=LOGDIR/f'{SID}.jsonl'
rows=[
 {'type':'event_msg','ts':1.0,'payload':{'type':'user_message','message':'photo proof fixture'}},
...
sidecar={
 ...
 # Stage-only photo attach should work without key-write support; confirmed send still works.
