...
SOCKS = APP/'socks'
LOGDIR = HOME/'batch-blocker-proof-logs'
CWD = HOME/'batch-blocker-proof-repo'
CALLS = HOME/'batch-blocker-proof-calls.jsonl'
SID='batch-blocker-proof'
for p in (SOCKS, LOGDIR, CWD): p.mkdir(parents=True, exist_ok=True)
(CWD/'README.md').write_text('batch blocker proof repo\n', encoding='utf-8')
log_path=LOGDIR/f'{SID}.jsonl'
rows=[
 {'type':'event_msg','ts':1.0,'payload':{'type':'user_message','message':'batch blocker fixture'}},
...
def staged_file_count() -> int:
    root = APP/'uploads'/SID
    if not root.exists():
        return 0
    return sum(1 for p in root.iterdir() if p.is_file() or p.is_symlink())
def current_state():
    out=dict(state)
...
def log(req, resp):
    with CALLS.open('a', encoding='utf-8') as f:
        f.write(json.dumps({'ts':time.time(),'req':req,'resp':resp,'staged_file_count':staged_file_count()})+'\n')
...
def loop():
  while True:
    ...
    try:
      ...
      cmd=req.get('cmd')
      if cmd=='state': resp=current_state()
      elif cmd=='tail': resp={'tail':''}
      elif cmd=='send':
        text=str(req.get('text') or '')
        resp={'commit_unknown':True,'error':'forced unknown'} if 'FORCE_COMMIT_UNKNOWN' in text else {'queued':False,'queue_len':0,'busy':True}
