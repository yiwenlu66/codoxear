...
- Producer UX polish: re-check blockers per file in long batches if needed, and decide whether the desktop/no-camera capture button should remain a single-image picker.

## Current justified claim
The staged upload expansion slice is accepted through multi-file picker, paste/drop, capture/camera, post-confirmed-send tail isolation, and public path redaction: attachments are server-staged before send, visibly manageable in the browser without exposing backend upload paths, committed to the backend only at confirmed send, and never turned into retry-inviting send failures by covered post-delivery cleanup/projection errors.
