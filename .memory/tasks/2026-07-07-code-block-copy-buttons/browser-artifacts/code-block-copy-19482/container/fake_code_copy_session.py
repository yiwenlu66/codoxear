...
SOCKS = APP / "socks"
SID = "code-copy-session"
SOCK = SOCKS / f"{SID}.sock"
BASE = HOME / "code-copy-proof"
LOG = BASE / "code-copy-session.jsonl"
CWD = BASE / "workspace"
CALLS = BASE / "broker-calls.jsonl"
FAKE_NOTICE = "FAKE_CODE_BLOCK_COPY_PROOF_DOCKER_ONLY"

MARKDOWN = """Here are two independent snippets.
...
def write_files() -> None:
    ...
    (CWD / "README.md").write_text("code block copy browser proof\n", encoding="utf-8")
    rows = [
        {"type": "event_msg", "payload": {"type": "user_message", "message": "show code copy proof"}, "ts": 1.0},
        {
            "type": "response_item",
            "payload": {
                "type": "message",
...
def serve() -> None:
    ...
    while True:
        ...
        try:
            ...
            print(f"fake code copy broker error: {exc}", file=sys.stderr, flush=True)
        finally:
            try:
                conn.close()
            except Exception:
