...
- **Residual generation risk after the B1 corrections:** the ordered boundary identifies a log generation by path/device/inode and detects truncation only while the observed size remains below the prior committed end. A same-inode truncate-and-rewrite that regrows to or beyond that end before the next observation can therefore be accepted as forward progress even though its prefix belongs to replacement history. Closing this correctly requires carrying a read-coherent prefix fingerprint in every observation, comparing it with both the current file and the prior committed boundary, and forcing a full re-projection when an incremental observation discovers a mismatch; a commit-time hash alone could apply stale parsed fields after a concurrent rewrite. That change expands the observation contract across discovery, refresh, listing, background scanning, polling, and SSE, so it remains outside the contained B1/B3 correction set rather than adding a partial guard with false append resets.
