...
Issue #79 source-string-test audit: classified all 77 `tests/*source*.py` files individually. 41 read JavaScript only to execute it in a Node VM harness and assert observable runtime state; 36 execute Python/API behavior directly. None asserts raw production source text. A broader test-suite audit found five true source-structure test files outside that legacy filename set: `tests/test_pytest_config.py`, `tests/test_extracted_module_hygiene.py`, `tests/test_session_manager_composition.py`, `tests/test_session_manager_factories.py`, and `tests/test_server_route_deps_caps.py`. Replaced them with runtime checks (pytest subprocess configuration behavior, Node VM exports, manager coordinator delegation, runtime factory capability construction, and route-dependency factory outputs). Parsed CSS declaration tests and filesystem-output `read_text()` assertions remain behavioral by contract. Focused replacement tests passed (5), and all 77 legacy `*source*.py` tests passed (194 + 3 subtests). Docker verification is pending before commit.

--- [2026-08-06T06:18:00Z] ---

Docker preflight passed in the isolated sandbox (`CODOXEAR_DOCKER_PORT=19879`, `/tmp/codoxear-issue79-docker-19879`); image-build verification was initially blocked by Docker-socket permissions. A sudo Docker wrapper reached the isolated container, where the run exposed a real read-only-workspace defect in the new pytest configuration test: it tried to write a probe module under `/workspace`. Replaced that with a temporary external test that imports the actual checkout `codoxear` package and asserts its resolved path; this tests the same pytest `pythonpath` behavior without mutating the mounted checkout. Docker could not complete under the available ordinary Docker access; parent steering authorized commit after local validation.

--- [2026-08-06T08:26:49+08:00] ---
Docker-isolated visual verification ran only against committed `c3ce16b766efbbd442ec485689bc45c073a3b4d9` at loopback `127.0.0.1:19643`; no service on `:8743` was contacted. `scripts/docker_verify.sh HEAD` passed its authenticated application bootstrap checks. A retained isolated container supplied a Pi session for `agent-browser` inspection at 390×844 and 1280×800. Mobile: document/body scroll width were both 390; sidebar and one active card were visible after opening the drawer; nav buttons were visible 32px controls at x=272/310/346; seeded authoritative token data produced a 32px `#ctxChip` matching adjacent 32px chrome; the date separator was 126.7×27.2px, transparent, and compact; composer was 53px. Desktop: sidebar ended at x=380 and chat began at x=380 (no gap), chat width was 900px, active card had a visible `1px solid rgb(47, 43, 38)` border plus 1px outline, and desktop card actions transitioned from opacity 0/pointer-events none/hidden to opacity 1/pointer-events auto/visible on hover. Screenshots: `/tmp/mobile-verify.png`, `/tmp/desktop-verify.png`; generic harness artifacts: `/tmp/codoxear-docker-verify-results.LbA448/`.
