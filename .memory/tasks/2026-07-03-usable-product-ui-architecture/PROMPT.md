...
4. **Certify the Workbench surfaces below.** File/git and mobile companion use are first-class Workbench surfaces, not optional polish. Browser evidence must cover them explicitly.
...
## Workbench
The Workbench is the product map. Each item below must be certified from the browser in an isolated Docker deployment unless explicitly marked as code-only.
...
13. **Acceptance**
   ...
   - Independent clean-room review scoped to product truthfulness, Workbench completeness, and architecture authority, not just test mechanics.
...
- Prior task `.memory/tasks/2026-06-11-major-refactor-new-features/` is complete (8 Workbench items + challenged-review fixes). Prior task `.memory/tasks/2026-06-12-structural-refactor-ux-review/` is parked; its mandatory browser-review requirement folds into this task's Workbench.
