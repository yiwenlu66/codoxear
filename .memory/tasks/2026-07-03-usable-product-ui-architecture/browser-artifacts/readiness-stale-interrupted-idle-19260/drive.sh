#!/usr/bin/env bash
# Drive the send/queue/readiness divergence discriminator on port 19260.
# Phases 1-3 establish the listing/sidebar busy divergence (broker keeps
# interrupted_idle:true while a post-interrupt user row lands on the log).
# Phases 4-5 probe the DIRECT send and QUEUE promotion routes to see whether
# the readiness path bypasses the listing suppression.
set -u
PORT=19260
...
print(json.dumps({'total':len(calls),'calls':[{'cmd':c['cmd'],'ts':c['ts']} for c in calls]}))
"
}
...
HTTP_CODE=$(curl -sS -o $REPO_ART/phase4-send-body.txt -w '%{http_code}' \
  ...
  -d '{"text":"probe direct send while sidebar busy"}' \
...
echo ""
echo "============================================================"
echo "PHASE 5: QUEUE DISCRIMINATOR"
echo "  POST /api/sessions/<sid>/enqueue"
echo "  EXPECTED (PASS): item stays queued, broker receives NO send attempt"
echo "  DEFECT: broker records a cmd:send (queue promotion) while busy"
echo "============================================================"
...
HTTP_CODE_Q=$(curl -sS -o $REPO_ART/phase5-enqueue-body.txt -w '%{http_code}' \
  -b $JAR -H 'Content-Type: application/json' \
  -d '{"text":"probe queue promotion while sidebar busy"}' \
...
echo ""
echo "============================================================"
echo "FINAL: full call log dump"
echo "============================================================"
call_log_dump > $REPO_ART/broker-call-log.jsonl
cat $REPO_ART/broker-call-log.jsonl
echo ""
