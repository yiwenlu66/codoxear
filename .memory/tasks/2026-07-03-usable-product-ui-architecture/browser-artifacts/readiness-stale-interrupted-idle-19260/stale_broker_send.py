#!/usr/bin/env python3
"""Stale fake broker for the send/queue/readiness divergence discriminator.

Reports a PERMANENT stale ``interrupted_idle=true`` (busy:false, queue_len:0)
so the listing/sidebar suppression path clears the override while the raw
broker state still carries ``interrupted_idle:true``.

Logs EVERY command received to /tmp/stale_broker_calls.jsonl so the route
driver can prove whether the server actually attempted a confirmed send
(``cmd:send``) or key-write (``cmd:keys``) while the sidebar reported busy.

Implements ``cmd:state``, ``cmd:tail``, ``cmd:send`` (confirmed sync send),
``cmd:keys``, ``cmd:shutdown``. Writes socket + sidecar + initial
interrupted-turn Codex rollout log under the container app dir so the REAL
server discovers them. No product code is stubbed.
"""
...
CALLLOG = Path("/tmp/stale_broker_calls.jsonl")
...
def handle(conn):
    try:
        ...
        if cmd == "state":
            ...
            # ONLY that the server reached this point.
