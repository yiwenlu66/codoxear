#!/usr/bin/env bash
# Clean queue discriminator (FIXED HEAD 206fb6c): fresh session (SID cert-stale-q),
# queue probed BEFORE any direct send, so no leftover send-boundary can mask the
# divergence. EXPECTED (PASS): item stays queued, broker receives NO cmd:send.
set -u
PORT=19264
SID=cert-stale-q
...
HTTP=$(curl -sS -o $REPO_ART/phaseQ5-enqueue-body.txt -w '%{http_code}' \
  ...
  -d '{"text":"queue probe on fresh busy session FIXED"}' \
...
# Dump broker 2 call log
docker exec $CONT cat /tmp/stale_broker_calls_q.jsonl 2>/dev/null > $REPO_ART/broker2-call-log.jsonl || true
echo "broker2 call log dump:"; cat $REPO_ART/broker2-call-log.jsonl
