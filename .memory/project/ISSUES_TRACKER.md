...
| 22 | Web-sent /new strands binding on old log + false busy | CLOSED | 1ad5e318, f5548542 | Two defects: (A) broker send latch assumed every send starts a log turn and the clear gate required log-driven completion evidence — fixed by recording log_off at send and clearing when no growth past the quiet window, plus closing an unfulfilled latch at a binding switch whose seed shows no open turn; (B) Pi defers new-session file creation until the first assistant row — fixed by the bridge pre-flushing Pi's own header object + flipped flushed flag on reason "new" (guarded, degrades to deferred binding). Docker-verified at f5548542: rebind+busy-clear+transcript-clear all before the first message. Residual: /name-style commands that write non-turn log entries still latch until the next real turn |
...
| 23 | False busy after webui model switch (Pi) | CLOSED | 9242aa6c | Send latch bet was byte-growth-based: Pi `model_change`/`thinking_level_change` rows grew the log without a turn, wedging the latch forever; the send had also cleared the interrupted-idle projection, so a non-final (errored) log tail displayed busy permanently. Fix: turn-aware latch (`send_latch_activity_ts`) + settled-without-turn latch restores the idle projection. Closes #22's documented residual. Also fixed latent sessiond sweep AttributeError (State lacked send-latch fields). Docker-verified both directions (scripts/docker_model_switch_busy.sh): HEAD~1 stays busy after a browser /model send on an error-tailed session; HEAD settles idle in 3 polls and the sidebar dot renders idle |
