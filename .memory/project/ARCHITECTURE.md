...
- **Frontend**: `app.js` is the app shell/wiring. Stateful subsystems live in `app_file_viewer.js`, `app_file_editor.js`, `app_file_picker.js`, `app_transcript.js`, `app_message_rows.js`, `app_launch.js`, `app_new_session.js`, `app_queue.js`, `app_diagnostics.js`, `app_recovery.js`, `app_unattended.js`, plus helper modules. Ownership pattern: controller modules own their state/actions/rendering; app.js owns DOM construction when still coupled to the shell and delegates through thin wrappers with fail-loud module checks. Remaining app.js concentrations: chat search/navigation orchestration and voice/notifications.
