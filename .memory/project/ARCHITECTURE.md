...
- **Frontend**: `app.js` (~9k lines) is the app shell/wiring; stateful subsystems live in `app_file_viewer.js`, `app_file_editor.js`, `app_file_picker.js`, `app_transcript.js`, `app_message_rows.js`, `app_launch.js`, plus helper modules. Remaining app.js concentrations: chat search/navigation orchestration, new-session dialog, queue/recovery panels, voice/notifications.
