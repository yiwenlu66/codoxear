...
class TestBackendSeamsSource(unittest.TestCase):
    ...
    def test_server_uses_payload_sidebar_and_pi_bridge_seams(self) -> None:
        ...
        self.assertIn("from .sessions import creation as _session_creation", source)
    ...
    def test_server_delegates_session_control_send_and_queue_flows(self) -> None:
        ...
        self.assertIn("return _session_creation.parse_create_session_request(RUNTIME, obj)", source)
        self.assertIn("return _session_creation.read_codex_launch_defaults(RUNTIME)", source)
        self.assertIn("return _session_creation.read_pi_launch_defaults(", source)
        self.assertIn("return _session_creation.read_new_session_defaults(", source)
        self.assertIn("return _session_listing.session_row_display_name(row, fallback=fallback)", source)
        self.assertIn("return _session_listing.normalize_session_cwd_row(RUNTIME, row)", source)
        self.assertIn("return _session_listing.frontend_session_list_row(RUNTIME, row)", source)
        self.assertIn("return _session_listing.session_list_group_key(RUNTIME, row)", source)
        self.assertIn("return _session_listing.session_list_payload(", source)
        self.assertIn("return _session_listing.historical_sidebar_items(", source)
        self.assertIn("return _session_listing.resume_preview_from_text(text, max_chars=max_chars)", source)
        self.assertIn("return _session_listing.first_user_message_preview_from_log(", source)
        self.assertIn("return _session_listing.first_user_message_preview_from_pi_session(", source)
