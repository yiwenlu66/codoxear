...
def test_session_manager_routes_operations_to_their_runtime_coordinators() -> None:
    """Public manager operations preserve values and return their owner result."""
    ...
    cases = (
        ...
        (
            "_queue_coordinator_for_manager",
            "delete_local",
            "queue_delete",
            ("sid", "item"),
            {"allow_commit_unknown": True},
            {"allow_commit_unknown": True, "allow_orphan_recovery": False},
            "deleted",
        ),
        (
            "_unattended_config_coordinator_for_manager",
            "set",
            "unattended_set",
            ("sid",),
            {"enabled": True},
            {"enabled": True, "request": None, "cooldown_minutes": None, "remaining_injections": None},
            "saved",
        ),
        ("_control_coordinator_for_manager", "get_state", "get_state", ("sid",), {}, {}, {"busy": False}),
    )

    for factory_name, coordinator_method, manager_method, args, call_kwargs, forwarded_kwargs, expected in cases:
        ...
        assert getattr(manager, manager_method)(*args, **call_kwargs) == expected
        assert calls == [(args, forwarded_kwargs)]
