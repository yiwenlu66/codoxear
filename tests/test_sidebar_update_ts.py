...
class TestSidebarUpdateTimestamp(unittest.TestCase):
    ...
    def test_mark_log_delta_does_not_advance_sidebar_ts_on_turn_complete(self) -> None:
        mgr = SessionManager.__new__(SessionManager)
        mgr._lock = threading.Lock()
        mgr._sessions = {}
        session = Session(
    ...
    def test_mark_log_delta_does_not_advance_sidebar_ts_for_tool_event(self) -> None:
        mgr = SessionManager.__new__(SessionManager)
        mgr._lock = threading.Lock()
        mgr._sessions = {}
        session = Session(
