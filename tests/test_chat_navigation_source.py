...
class TestChatNavigationSource(unittest.TestCase):
    ...
    def test_visible_time_indicator_uses_first_visible_message(self) -> None:
        ...
        self.assertIn("isSearchOpen: () => loadedChatSearchSnapshot().open,", source)
    ...
    def test_loaded_chat_search_is_rendered_row_scoped_with_all_transcript_count(self) -> None:
        ...
        self.assertIn('shouldCancelOlderLoad: () => olderLoadRuntime.shouldCancelOnScroll(),', source)
        self.assertIn('if (shouldCancelOlderLoad() && cur > olderCancelPx) cancelOlderLoad();', APP_TRANSCRIPT_JS.read_text(encoding='utf-8'))
        ...
        self.assertNotIn('let chatSearchAllCountTruncated = false;', source)
        self.assertNotIn('let chatSearchAllTimer = null;', source)
        self.assertIn('const chatSearchAllRuntime = codoxearTranscript.createChatSearchAllRuntime({', source)
        self.assertIn('function scheduleAllChatSearchCount(query)', source)
        self.assertIn('chatSearchAllRuntime.schedule(cleanQuery, (scheduledQuery) => {', source)
        self.assertIn('void refreshAllChatSearchCount(scheduledQuery);', source)
        ...
        self.assertNotIn('let chatSearchAllHint = "";', source)
        ...
        self.assertIn('chatSearchAllRuntime.completeRequest(request, {', source)
