...
class TestChatNavigationSource(unittest.TestCase):
    ...
    def test_loaded_chat_search_is_rendered_row_scoped_with_all_transcript_count(self) -> None:
        ...
        self.assertIn('if (refreshAllCount) scheduleAllChatSearchCount(query);', source)
        self.assertIn('refreshLoadedChatSearch({ jump: false, preserveCurrent: true, refreshAllCount: false });', source)
