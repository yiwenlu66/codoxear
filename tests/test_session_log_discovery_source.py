...
class TestSessionLogDiscoverySource(unittest.TestCase):
    def test_session_log_discovery_has_dedicated_owner_with_util_facade(self) -> None:
        ...
        self.assertIn("for p in sessions_dir.rglob(backend.log_glob_pattern()):", discovery_source)
        self.assertIn("if not backend.is_session_log_path(p, sessions_dir=sessions_dir):", discovery_source)
        self.assertIn("def find_session_log_for_session_id(", discovery_source)
        self.assertIn("if backend.log_matches_session_id(p, session_id):", discovery_source)
        self.assertIn("sid = backend.session_id_from_payload_or_log(p, payload)", discovery_source)
        self.assertNotIn("read_pi_session_id(p)", discovery_source)
        self.assertNotIn("read_cc_session_id(p)", discovery_source)
