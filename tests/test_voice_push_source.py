...
def eval_desktop_notification() -> dict:
    sources = [path.read_text(encoding="utf-8") for path in (MODAL, HELPERS, NOTIFICATIONS, VOICE)]
    script = textwrap.dedent(
        ...
        function node(){{return {{style:{{}},dataset:{{}},title:"",disabled:false,value:"",checked:false,textContent:"",classList:{{add(){{}},remove(){{}},toggle(){{}}}},setAttribute(){{}},append(){{}},appendChild(c){{return c}},replaceChildren(){{}},addEventListener(){{}},matches:()=>false,focus(){{}}}}}}
        const ctx={{HTMLElement,Notification,window:{{isSecureContext:true,focus:()=>focused.push("window")}},navigator:{{userAgent:"X11"}},document:{{activeElement:null,contains:()=>true,createElement:()=>node()}},requestAnimationFrame:f=>f(),setTimeout:()=>1,clearTimeout(){{}},setInterval:()=>1,clearInterval(){{}}}};
        vm.createContext(ctx);for(const source of {json.dumps(sources)})vm.runInContext(source,ctx);
        const d={{announceBtn:node(),notificationBtn:node(),liveAudio:node(),voiceSettingsStatus:node(),voiceBaseUrlInput:node(),voiceApiKeyInput:node(),voiceClearApiKeyToggle:node(),narrationSettingToggle:node(),voiceSettingsCancelBtn:node(),voiceSettingsSaveBtn:node(),notificationOptions:{{root:{{appendChild(){{}}}},voiceHost:{{style:{{}},firstChild:null,appendChild(){{}},insertBefore(){{}}}},el:()=>node(),iconSvg:()=>"",isAppDisposed:()=>false,api:async u=>String(u).includes("feed")?{{items:[{{message_id:"m1",session_display_name:"Repo",notification_text:" hello   world ",session_id:"s1",updated_ts:1}}]}}:{{subscriptions:[]}},setToast(){{}},handleAppAuthLoss(){{}},resolveAppUrl:x=>x,versionedShellAssetPath:x=>x,storageGetItem:k=>k==="codoxear.notificationEnabled"?"1":k==="codoxear.desktopNotificationsEnabled"?"1":null,storageSetItem(){{}},storageRemoveItem(){{}},eventBindings:{{on(t,y,h){{t["on"+y]=h;return h}}}},focusSessionFromNotification:s=>focused.push(s),windowTarget:ctx.window,navigatorTarget:ctx.navigator,documentTarget:ctx.document,Notification,clearTimeout(){{}}}},eventBindings:{{on(t,y,h){{t["on"+y]=h;return h}}}},isAppDisposed:()=>false,api:async u=>String(u).includes("feed")?{{items:[{{message_id:"m1",session_display_name:"Repo",notification_text:" hello   world ",session_id:"s1",updated_ts:1}}]}}:{{}},setToast(){{}},handleAppAuthLoss(){{}},openSettings(){{}},closeSettings(){{}},resolveAppUrl:x=>x,versionedShellAssetPath:x=>x,storageGetItem:k=>k==="codoxear.notificationEnabled"?"1":k==="codoxear.desktopNotificationsEnabled"?"1":null,storageSetItem(){{}},storageRemoveItem(){{}},focusSessionFromNotification:s=>focused.push(s),requestFrame:f=>f(),setTimeout:()=>1,clearTimeout(){{}},setInterval:()=>1,clearInterval(){{}}}};
        (async()=>{{const c=ctx.window.CodoxearVoice.createVoiceController(d);await c.refreshBackgroundState({{force:true}});notifications[0].onclick({{preventDefault(){{}}}});process.stdout.write(JSON.stringify({{title:notifications[0].title,body:notifications[0].options.body,tag:notifications[0].options.tag,focused,closed:notifications[0].closed}}))}})().catch(e=>{{console.error(e);process.exit(1)}})
        """
    )
