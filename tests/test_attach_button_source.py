...
class TestAttachButtonSource(unittest.TestCase):
    def test_attach_button_reflects_session_selection(self) -> None:
        ...
        self.assertIn('const captureLabel = attachBlocker || `Add photo (max ${fmtBytes(ATTACH_UPLOAD_MAX_BYTES)})`;', source)
    ...
    def test_attach_upload_uses_shared_stage_files_pipeline(self) -> None:
        ...
        self.assertIn('let uploadName = f.name || (producer === "paste" ? pastedFileName(f, fileIndex, producerNameSeed) : producer === "capture" ? photoFileName(f, fileIndex, producerNameSeed) : "file");', source)
    ...
    def test_capture_producer_routes_through_stage_files(self) -> None:
        ...
        self.assertIn('title: "Add photo",', source)
        self.assertIn('"aria-label": "Add photo",', source)
        self.assertNotIn('title: "Capture photo",', source)
        self.assertNotIn('"aria-label": "Capture photo",', source)
        ...
        self.assertIn('function imageExtensionFromMimeType(type, fallback = "") {', source)
        self.assertIn('if (normalized === "image/jpeg" || normalized === "image/jpg") return "jpg";', source)
        self.assertIn('if (normalized === "image/png") return "png";', source)
        self.assertIn('if (normalized === "image/gif") return "gif";', source)
        self.assertIn('if (normalized === "image/webp") return "webp";', source)
        self.assertIn('return normalized.startsWith("image/") ? fallback : "";', source)
        self.assertIn('function photoFileName(file, index, seed) {', source)
        self.assertIn('const ext = imageExtensionFromMimeType(file && file.type, "jpg") || "jpg";', source)
        self.assertIn('return `photo-${seed}${suffix}.${ext}`;', source)
        self.assertIn('producer === "capture" ? photoFileName(f, fileIndex, producerNameSeed) : "file"', source)
        self.assertIn('producer === "capture" ? "staging photo..." : "uploading file..."', source)
        self.assertNotIn('staging captured photo', source)
        self.assertNotIn('captured-${seed}', source)
