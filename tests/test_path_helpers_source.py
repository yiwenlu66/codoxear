...
class TestPathHelpersSource(unittest.TestCase):
    def test_path_matching_and_rollout_session_id_are_centralized(self) -> None:
        ...
        path_source = sources[ROOT / "codoxear" / "session_log_paths.py"]
        self.assertIn("def _paths_match(a: Path, b: Path) -> bool:", path_source)
        self.assertIn("def session_id_from_rollout_path(log_path: Path) -> str | None:", path_source)
        self.assertIn("from .session_log_paths import _paths_match", util_source)
        self.assertIn("from .session_log_paths import session_id_from_rollout_path", util_source)
        self.assertNotIn("def _paths_match(a: Path, b: Path) -> bool:", util_source)
        self.assertNotIn("def session_id_from_rollout_path(log_path: Path) -> str | None:", util_source)
