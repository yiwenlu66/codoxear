...
def eval_open_session_tail_request_abort() -> dict:
    ...
    js = textwrap.dedent(
        ...
        const messageFlow = {{
          ...
          isCurrentOpenSessionTailRequest: (request) => sessionState.get("selected") === request.sessionId && state.pollGen === request.generation,
...
class TestChatScrollbackSource(unittest.TestCase):
    ...
    def test_clear_selected_session_after_removal_resets_missing_session_state(self) -> None:
        ...
        for expected in [
            ...
            ["clearAttachments"],
            ["resetChatRenderState"], ["hideUnattendedMenu"],
        ]:
