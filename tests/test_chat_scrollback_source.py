...
class TestChatScrollbackSource(unittest.TestCase):
    ...
    def test_new_command_begins_transcript_renewal_after_send_ack(self) -> None:
        ...
        self.assertIn("const confirmed = await confirmApp({", block)
        self.assertIn("message: \"This session has a pending file attachment. Send it with this message?\"", block)
        self.assertLess(block.index("await confirmApp"), block.index("transcriptEventRuntime.addPendingUser"))
