...
class TestVoicePlaybackSource(unittest.TestCase):
    ...
    def test_live_audio_watchdog_detects_non_progress_and_hard_resets(self) -> None:
        ...
        self.assertIn('addAppEvent(document, "visibilitychange"', source)
        self.assertIn('addAppEvent(window, "pageshow"', source)
        self.assertIn('addAppEvent(window, "online"', source)
        self.assertIn('addAppEvent(window, "focus"', source)
