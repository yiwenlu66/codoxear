...
class TestFileUploadModuleSource(unittest.TestCase):
    ...
    def test_inject_file_route_checks_session_idle_before_staging(self) -> None:
        ...
        self.assertIn('{"error": "wait for the current queue drain before attaching a file"}', block)
        ...
        self.assertNotIn("require_key_write_errors", readiness_source)
        self.assertNotIn("attachment_injection_ready", readiness_source)
        self.assertIn("def attachment_staging_ready(self, session_id: str) -> bool:", readiness_source)
        self.assertNotIn("inject_attachment_keys", route_source + readiness_source)
