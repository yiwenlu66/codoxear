...
class ServerRouteDepsFactory:
    ...
    def control_route_deps(self) -> ControlRouteDeps:
        ...
        return ControlRouteDeps(
            ...
            attach_upload_body_max_bytes=self.config.ATTACH_UPLOAD_BODY_MAX_BYTES,
            attach_upload_max_bytes=self.config.ATTACH_UPLOAD_MAX_BYTES,
    ...
    def session_route_deps(self) -> SessionRouteDeps:
        ...
        return SessionRouteDeps(
            ...
            default_agent_backend=self.config.DEFAULT_AGENT_BACKEND,
    ...
    def voice_route_deps(self) -> VoiceRouteDeps:
        ...
        return VoiceRouteDeps(
            ...
            json_response=server._json_response,
            read_json_body=lambda handler, **kwargs: handler._read_json_body(**kwargs),
            load_unattended_prompt=lambda: server._load_unattended_prompt(server.UNATTENDED_PROMPT_PATH),
            save_unattended_prompt=lambda prompt: server._save_unattended_prompt(server.UNATTENDED_PROMPT_PATH, prompt),
            default_unattended_prompt=server.UNATTENDED_PROMPT_PREFIX,
        )
    ...
    def git_route_deps(self) -> GitRouteDeps:
        ...
        return GitRouteDeps(
            ...
            git_changed_files_max=self.config.GIT_CHANGED_FILES_MAX,
            git_diff_timeout_seconds=self.config.GIT_DIFF_TIMEOUT_SECONDS,
            git_diff_max_bytes=self.config.GIT_DIFF_MAX_BYTES,
