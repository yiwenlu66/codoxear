(function () {
  ...
  function createMessageFlowController(options = {}) {
    ...
    function scheduleMessageEventSourceRetry(sessionId, generation) {
      if (!isCurrent(sessionId, generation) || messageSseRetryTimer) return;
      const delay = Math.max(1000, messageSseFallbackUntil - now());
      messageSseRetryTimer = setTimeoutFn(() => {
        messageSseRetryTimer = null;
        if (isCurrent(sessionId, generation)) openMessageEventSource(sessionId, generation);
      }, delay);
    }

    function openMessageEventSource(sessionId = getSelected(), generation = getGeneration()) {
      if (!sessionId || !isCurrent(sessionId, generation) || typeof EventSourceCtor !== "function") return;
