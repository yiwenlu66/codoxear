...
const FILE_CANDIDATE_CACHE_TTL_MS = 15000;
    ...
    const filePickerController = filePickerOpsModule.createFilePickerOpsController({
      wiring, codoxearFilePicker, normalizeLineNumber, filePickerField, filePickerMenu, filePickerInput, api, document, el,
      hooks: {
        getSelected, blockUnavailableFileAction, currentFileViewerSessionId,
        fileViewerController: () => fileViewerController, fileCandidateKey, currentActiveFileDraft, activeFilePathValue,
        normalizeFileApiPath, renderFilePickerMenu, applyFileMenuState, normalizeDraftFilePath,
        filePickerSectionLabel, duplicateFilePickerPaths, rawByteDuplicatePaths, filePickerIdentityHint, filePickerTitle,
        currentActiveFileIdentity, openDraftFilePathWithGuard, openFilePathWithResolvedMode, filePickerSelectionLine,
    ...
    const fileEditorOpsController = fileEditorOpsModule.createFileEditorOpsController({
      ...
      hooks: {
        ...
        disposeFileEditor: () => disposeFileEditor(), currentEditorKind: () => currentFileEditorKind(),
        setEditorKind: (kind) => setFileEditorKind(kind), currentFileEditMode: () => currentFileEditMode(),
        currentActiveFileEditable: () => currentActiveFileEditable(), isUnavailable: () => isFileViewerSessionUnavailable(),
        isProgrammaticChange: () => fileViewerController.isFileEditorProgrammaticChange(),
        currentTouchSelectMode: () => currentFileTouchSelectMode(), resetTouchSelectionState: () => resetFileTouchSelectionState(),
        currentActiveFileText: () => currentActiveFileText(), setDirty: (dirty) => setFileDirty(dirty),
        runProgrammaticChange: (callback) => fileViewerController.runFileEditorProgrammaticChange(callback),
        syncReadOnly: () => syncFileEditorReadOnly(), updateTouchToolbar: () => updateFileTouchToolbar(),
      },
    });
...
const filePickerOperations = filePickerOpsModule.createFilePickerOperationDelegates({
  fileViewerController, fileModeControlsRuntime, filePickerDomRuntime, filePickerMenuState, filePickerInput,
  filePickerInputRuntime, activeFilePathValue, openedFileRuntime, fileReferenceRuntime, filePickerSearchState,
  filePickerRenderRuntime, fileViewerPanelRuntime, getSelected, getSessionIndex, stripPathLocationSuffix,
});
...
filePickerOpsBinding.bindFilePickerInteractions({
  eventBindings, fileBtn, showFileViewer, filePickerInput, filePickerInputRuntime, fileModeDiffBtn, fileModePreviewBtn,
  fileEditBtn, handleFileDiffModeButtonPress, handleFilePreviewModeButtonPress, handleFileEditButtonPress,
  fileVideoPreviewBtn, fileVideoPreviewRuntime, fileDownloadBtn, fileDownloadRuntime, activeFileDownloadApiPath,
  codoxearFileViewer, fileTouchSelectBtn, fileTouchCopyBtn, fileTouchPasteBtn, fileTouchUpBtn, fileTouchLeftBtn,
  fileTouchDownBtn, fileTouchRightBtn, toggleFileTouchSelectionMode, copyActiveFileSelection,
  pasteFromClipboardIntoActiveFile, handleFileTouchMoveButtonPress, fileCloseBtn, fileBackdrop, requestHideFileViewer,
  $, fileUnsavedController, fileUnsavedBackdrop, filePasteInput, handleFilePasteInsert, hideFilePasteDialog,
  filePasteBackdrop, chatInner, codeBlockCopyRuntime, fileReferenceRuntime, fileDiff, addAppEvent, document, Element,
  isFileViewerOpen, menuState: filePickerMenuState, closeFilePickerMenu,
});
