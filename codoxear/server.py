...
class SessionManager:
    ...
    def get_ui_state(self, session_id: str) -> dict[str, Any]:
        ...
        if resp.get("error") == "unknown cmd":
            if _session_supports_live_pi_ui(s):
                raise ValueError(
                    "live ui interactions are unavailable for this pi session"
                )
    ...
    def get_session_commands(self, session_id: str) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            now_ts = time.time()
            session_path_key = (
                str(s.session_path.resolve())
                if isinstance(s.session_path, Path)
                else None
            )
            ...
            if isinstance(cached, dict):
                ...
                if (
                    isinstance(cached_ts, (int, float))
                    and (now_ts - float(cached_ts)) < PI_COMMANDS_CACHE_TTL_SECONDS
                ):
                    if (
                        cached.get("thread_id") == s.thread_id
                        and cached.get("session_path") == session_path_key
                    ):
            ...
            thread_id = s.thread_id
        try:
            resp = self._sock_call(sock, {"cmd": "commands"}, timeout_s=2.0)
        except Exception:
            if not _pid_alive(s.broker_pid) and not _pid_alive(s.codex_pid):
                with self._lock:
                    self._sessions.pop(runtime_id, None)
        ...
        if resp.get("error") == "unknown cmd":
            # Older live brokers may not expose the read-only commands RPC yet.
            # Treat this as "feature unavailable" instead of surfacing a hard UI error.
            return {"commands": []}

        error = resp.get("error")
        if isinstance(error, str) and error:
            raise ValueError(error)
        payload = _sanitize_pi_commands_payload(resp)
        with self._lock:
            cache = getattr(self, "_pi_commands_cache", None)
        ...
        try:
            ...
            if path.startswith("/assets/") and not USE_LEGACY_WEB:
                ...
                if served_dist_dir is not None:
                    ...
                    if (
                        _is_path_within(served_dist_dir.resolve(), candidate)
            ...
            if path == "/api/settings/voice":
                if not _require_auth(self):
                    ...
                    return
                _json_response(
                    self, 200, {"ok": True, **MANAGER._voice_push.settings_snapshot()}
                )
                return
            ...
            if path == "/api/notifications/subscription":
                ...
                _json_response(
                    self,
                    200,
                    {"ok": True, **MANAGER._voice_push.subscriptions_snapshot()},
                )
                return
            ...
            if path == "/api/session_resume_candidates":
                ...
                for row in rows:
                    ...
                    alias = (
                        MANAGER.alias_get(sid) if isinstance(sid, str) and sid else ""
                    )
                    preview = ""
                    log_path_raw = row.get("log_path")
                    session_path_raw = row.get("session_path")
                    if isinstance(log_path_raw, str) and log_path_raw:
                        preview = _first_user_message_preview_from_log(
                            Path(log_path_raw)
                        )
            ...
            if path.startswith("/api/sessions/") and path.endswith("/workspace"):
                ...
                if not session_id:
                    ...
                    return
                try:
                    payload = _session_workspace_payload(MANAGER, session_id)
                except KeyError:
                    _json_response(self, 404, {"error": "unknown session"})
                    return
                except ValueError as e:
            ...
            if path.startswith("/api/sessions/") and path.endswith("/details"):
                ...
                if not session_id:
                    ...
                    return
                try:
                    payload = _session_details_payload(MANAGER, session_id)
                except KeyError:
                    _json_response(self, 404, {"error": "unknown session"})
                    return
                _json_response(self, 200, payload)
            ...
            if path.startswith("/api/sessions/") and path.endswith("/diagnostics"):
                ...
                base_priority = _clip01(
                    time_priority + float(sidebar_meta["priority_offset"])
                )
                blocked = sidebar_meta["dependency_session_id"] is not None
                snoozed = (
                    sidebar_meta["snooze_until"] is not None
            ...
            session_id = _match_session_route(path, "commands")
            if session_id is not None:
                if not _require_auth(self):
                    self._unauthorized()
                    return
                try:
                    payload = MANAGER.get_session_commands(session_id)
                except KeyError:
                    _json_response(self, 404, {"error": "unknown session"})
                    return
                except ValueError as e:
            ...
            if path.startswith("/api/sessions/") and path.endswith("/file/search"):
                ...
                if not base.is_absolute():
                    base = base.resolve()
                try:
                    result = _search_session_relative_files(
                        base, query=query_raw[0], limit=limit
                    )
            ...
            if path.startswith("/api/sessions/") and path.endswith(
                "/git/changed_files"
            ):
                if not _require_auth(self):
                    self._unauthorized()
                ...
                for path_key, vals in _parse_git_numstat(staged_numstat).items():
                    ...
                    prev["additions"] = (
                        None
                        if add_prev is None or add_new is None
                        else int(add_prev) + int(add_new)
                    )
                    prev["deletions"] = (
                        None
                        if del_prev is None or del_new is None
                        else int(del_prev) + int(del_new)
                    )
        ...
        try:
            ...
            if path == "/api/notifications/subscription/toggle":
                ...
                if not isinstance(enabled, bool):
                    ...
                    return
                try:
                    payload = MANAGER._voice_push.toggle_subscription(
            ...
            if path == "/api/notifications/test_push":
                if not _require_auth(self):
                    ...
                    return
                try:
                    payload = MANAGER._voice_push.send_test_push_notification(
            ...
            if path == "/api/audio/listener":
                ...
                if not isinstance(enabled, bool):
                    _json_response(self, 400, {"error": "enabled must be a boolean"})
                    return
                payload = MANAGER._voice_push.listener_heartbeat(
            ...
            if path == "/api/audio/test_announcement":
                if not _require_auth(self):
                    ...
                    return
                try:
                    payload = MANAGER._voice_push.enqueue_test_announcement(
                        session_display_name="Codoxear test"
                    )
                except ValueError as e:
                    _json_response(self, 400, {"error": str(e)})
            ...
            if session_id is not None:
                ...
                if create:
                    try:
                        ...
                        return
                    try:
                        size, next_version = _write_new_text_file_atomic(
                            p, text=text_raw
                        )
                        ...
                        if p.is_file():
                            try:
                                _current_text, _current_size, current_version = (
                                    _read_text_file_for_write(
                                        p, max_bytes=FILE_READ_MAX_BYTES
                                    )
                                )
                    ...
                    try:
                        _current_text, _current_size, current_version = (
                            _read_text_file_for_write(p, max_bytes=FILE_READ_MAX_BYTES)
                        )
                    except FileNotFoundError as e:
                        _json_response(self, 404, {"error": str(e)})
                        return
            ...
            if path.startswith("/api/sessions/") and path.endswith("/harness"):
                ...
                if "text" in obj:
                    _json_response(
                        self, 400, {"error": "unknown field: text (use request)"}
                    )
                    return
                enabled: bool | None
                ...
                if remaining_injections_raw is not None:
                    try:
                        remaining_injections = _clean_harness_remaining_injections(
                            remaining_injections_raw, allow_zero=True
                        )
                    except ValueError as e:
                        _json_response(self, 400, {"error": str(e)})
                        return
            ...
            if path.startswith("/api/sessions/") and (
                path.endswith("/inject_file") or path.endswith("/inject_image")
            ):
                if not _require_auth(self):
                    self._unauthorized()
                    return
                ...
                if not isinstance(filename, str) or (not filename.strip()):
                    raise ValueError("filename required")
                if isinstance(attachment_index, bool) or not isinstance(
                    attachment_index, int
                ):
                    _json_response(
                        self, 400, {"error": "attachment_index must be an integer"}
                    )
                    return
