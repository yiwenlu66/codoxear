...
The UI ships three theme families — **paper** (default), **clay**, and **slate** — each in light and dark, selected per device from the Settings dialog (`app_settings.js`) and applied by `app_theme.js`. The rules below split into cross-theme invariants every family must keep, and per-family languages a theme may express. Invariants are not preferences.
