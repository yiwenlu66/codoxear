...
- **Slash completion and live controls are backend-aware.** Typing `/` shows browser-safe commands from `slash_commands.py`. Pi `/model` uses its shared TUI command; Pi `/effort` (with `/thinking` alias) is bridge-provided; Pi `/new` is advertised because the transcript replacement boundary (see "State-authority principle") renders its effect correctly, while the other session-lifecycle commands (`/resume`, `/fork`, `/clone`) stay hidden because their TUI flows pick targets interactively. Claude Code `/model` and `/effort` pickers inject its native commands. Capable Codex sessions run their TUI against a broker-owned app-server Unix socket; `/model` and `/effort` call the typed experimental `thread/settings/update` API, while unsupported Codex versions advertise neither command.
