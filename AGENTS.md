...
- **Pi subagent activity has two projections.** `pi-subagent:` events become inline narration rows, while `util.scan_active_pi_subagents()` supplies one record per actually running lifecycle-v3 child step (parallel siblings remain distinct; pending future chain steps do not count). The sidebar shows only the compact `▸N` marker. The transcript's busy typing bubble and idle activity bubble both show the compact summary plus one automatically visible line per supplied active child; there is no disclosure toggle, popover, or synthesized “unknown” row. Each bubble is intrinsically content-sized up to the normal transcript-bubble maximum, and detail lines wrap rather than widen or clip the row. Each line includes only producer-backed role/model/tool telemetry, removes a provider prefix from qualified `provider/model` names, and labels cumulative usage `tokens`.
