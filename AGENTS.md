...
- **Widget rule: element + render + subscription in one module.** `app_topbar.js` owns the status chip, context chip, and interrupt button end-to-end: element creation, rendering, session-store subscriptions, and interactions. `app_notifications.js` owns notification runtime (`enabledLocally`/`syncState`/`pollFeed`/`dispose`). `app_help.js` owns the help overlay. `app_settings.js` owns the Settings dialog (sidebar Settings button → appearance controls; its “Voice & notifications” row hands off to the voice dialog in `app_voice.js`, which is unchanged). `app_theme.js` owns the theme surface. `app_conversation_copy.js` owns the conversation-copy formatter.
...
- **Theming.** Three families (paper/clay/slate) × light/dark, default paper + follow-system, chosen per device in the Settings dialog and applied by `app_theme.js`. See the “Design language” section below for the cross-theme invariants, the theme file contract, and each family’s rules.
...
- **Single writer per displayed value.** `app_theme.js` is the only writer of `<html data-theme>`, `<html data-mode>`, the theme `<link>`, the custom-CSS `<style>`, and `<meta name="theme-color">`; the inline boot script in `index.html` renders the same persisted state once before first paint. Settings only renders controller state and forwards intent.
