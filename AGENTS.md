...
- **Send busy-latch and /new materialization contracts.** The broker latches `busy`/`turn_open` optimistically on every confirmed send, betting the backend writes a turn to the bound log; the latch records the log offset at send time, and the idle sweep clears it when no growth appears within the quiet window (no-turn TUI commands like `/new` or `/reload`), while a binding switch closes an unfulfilled latch whose new-log seed shows no open turn. Pi defers creating a new session's log until the first assistant row, so the bridge pre-flushes Pi's own header object (and flips the manager's `flushed` flag) on lifecycle reason `new`, letting the broker rebind at `/new` time instead of at the first message.
