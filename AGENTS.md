...
- **Pi bridge capability is advertised at extension load.** The bridge writes a PID-bound `.caps` sibling of its active-session marker on every load, so `/reload` enables `/thinking` discovery without a session event.
