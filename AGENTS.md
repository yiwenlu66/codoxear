...
Warm editorial language: cream paper, terracotta accent (`--accent`, `--on-accent`), system sans UI with serif markdown headings (`--font-prose`), radii 6/12/18px, pill badges, circular state dots, soft elevation on floating surfaces. Structural rules: hairline chrome borders, accent-filled primaries and active icon buttons, an accent left rule on the active session, a bordered composer pill, serif heading weight. Focus rings are accent-colored.

### Slate theme

ChatGPT-like language: white/soft-gray (dark: `#212121`/`#171717`), system sans, radii 8/12/18px, pill badges, circular dots, monochrome ink primaries. Structural rules: ghost icon buttons (border only through hover fill), borderless sidebar rows with fill selection, flat assistant messages (user turns keep a filled bubble; error/warning variants keep their borders), a 24px composer pill, hairline dialog borders.
